...
export const AUTHORIZE_URL = `https://app.slack.com/oauth/v2/authorize?client_id=${env.SLACK_CLIENT_ID}&redirect_uri=${env.SLACK_REDIRECT_URI}&response_type=code&user_scope=chat:write`;
// #endregion
...
async function channelIdsToNamesPass(
	md: string,
	client: Slack.webApi.WebClient,
): Promise<string> {
	return replaceAsync(md, /<#([A-Z0-9]*)>/g, async (matches) => {
...
async function usergroupIdsToNamesPass(
	md: string,
	client: Slack.webApi.WebClient,
): Promise<string> {
	return replaceAsync(md, /<!subteam\^([A-Z0-9]*)>/g, async (matches) => {
		...
		return `<https://hackclub.slack.com/archives/${usergroupId}|@${usergroupName}>`;
	});
}
//
