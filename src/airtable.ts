...
let json;
try {
    json =JSON.parse(await readFile('./airtable.json', 'utf-8'))
} catch (error) {
    throw logger.fatal(`Failed to read airtable.json: ${error.message}`);
}

const airtableJson = await ZAirtableJsonSchema.safeParseAsync(JSON.parse(json));
if (airtableJson.error) {
    const error = fromError(airtableJson.error);
    throw logger.fatal(`Failed to parse airtable.json: ${error.message}`);
}
