...
async function sendResponseMessage(
	...
	logger.debug(
		`Sending response message to ${story.slackIdRollup.length} author(s) for story ${story.headline} (${story.id})`,
	);
	...
	const message =
		type === "Approved"
			? `Your story *${story.headline}* has been approved! :yay:\nExpect to see it in the next issue.`
			: `Your story *${story.headline}* has been rejected :blob_sad:\nPlease edit your story and try again.`;
